...
A standard GeForce RTX 5070 Ti has 16 GB GDDR7. Never write "48 GB RTX 5070 Ti" in the paper unless the frozen `nvidia-smi` manifest demonstrates multiple devices or another environment exposing that aggregate memory. The experiment code loads one model at a time.
...
The active two-family experiment uses `Qwen/Qwen3-8B` and `mistralai/Mistral-7B-Instruct-v0.3`. Use 4-bit NF4 and record exact Hugging Face snapshot SHAs before the frozen test run.
